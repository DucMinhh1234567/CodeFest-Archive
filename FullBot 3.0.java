...
public class Main {
    ...
    private static final String GAME_ID = "138684"; // Nhập Game ID vào đây
...
class FullBotListener implements Emitter.Listener {
    ...
    public void call(Object... args) {
        try {
            ...
            // Đồng bộ cooldown inventory mỗi lượt
            cooldownManager.syncInventory(hero.getInventory(), gameMap.getStepNumber());

            // === SMART SUPPORT ITEM USAGE ===
            smartUseSupportItems(gameMap, player);

            // === ITEM COLLECTING & SWAPPING (bao gồm cả chest breaking) ===
            handleItemCollectAndSwap(gameMap, player, getNodesToAvoid(gameMap));

            // === ENEMY HUNTING ===
            if (hasEnoughWeapons()) {
                handleEnemyHuntingAllWithCooldown(gameMap, player, getNodesToAvoid(gameMap));
            } else {
    ...
    private Element findNearestUpgradeItem(GameMap gameMap, Player player) {
        ...
        for (Element item : allItems) {
            if (!isItemInSafeZone(item, gameMap)) continue;
            String typeName = item.getClass().getSimpleName();
            if (typeName.equals("SupportItem") && hero.getInventory().getListSupportItem().size() >= 4) {
                // Đã đủ 4 support, bỏ qua support item
                continue;
            }
    ...
    private boolean isItemBetterThanCurrent(Element item) {
        ...
        return switch (itemType) {
            ...
            case "SUPPORT" -> hero.getInventory().getListSupportItem().size() < 4; // Chỉ nhặt support nếu chưa đủ 4 slot
            default -> false;
        };
    }
    ...
    // ================== ENEMY HUNTING (COOLDOWN) ==================
    private void handleEnemyHuntingAllWithCooldown(GameMap gameMap, Player player, List<Node> nodesToAvoid) throws IOException {
        // Ưu tiên xử lý đồ nếu có item gần
        ...
        Player enemy = findNearestEnemy(gameMap.getOtherPlayerInfo().stream().filter(e -> e.getHealth() > 0).toList(), player);
        if (enemy == null) {
            System.out.println("Không còn kẻ địch nào để đánh!");
            return;
        }
        int dist = PathUtils.distance(player, enemy);
        boolean close = dist <= 1;
        int step = gameMap.getStepNumber();
        Weapon gun = hero.getInventory().getGun();
        Weapon special = hero.getInventory().getSpecial();
        Weapon best = cooldownManager.chooseBest(step, close);

        // Nếu có GUN (không phải Shotgun) và kẻ địch vào tầm thì bắn ngay
        if (gun != null && !"SHOTGUN".equals(gun.getId())) {
            int[] gunRange = gun.getRange();
            int minRange = gunRange[0], maxRange = gunRange[1];
            String dir = getShootDirection(player, enemy);
            if (dist >= minRange && dist <= maxRange && dir != null && !isShootPathBlocked(player, enemy, dir, gameMap)) {
                hero.shoot(dir);
                cooldownManager.markUsed(gun, step);
                return;
            }
        }

        // Nếu có Special là ROPE và kẻ địch vào tầm thì dùng ngay
        if (special != null && "ROPE".equals(special.getId())) {
            int ropeRange = 6; // Theo logic ROPE
            String dir = getDirectionToEnemy(player, enemy);
            if (dist <= ropeRange && dir != null) {
                hero.useSpecial(dir);
                cooldownManager.markUsed(special, step);
                return;
            }
        }

        // Nếu không phải trường hợp trên, dùng logic cooldown như cũ
        Weapon throwable = hero.getInventory().getThrowable();
        Weapon melee = hero.getInventory().getMelee();
        if (best != null) {
            String dir = getDirectionToEnemy(player, enemy);
            switch (best.getType().name()) {
                case "MELEE" -> {
                    if (close && dir != null) {
                        hero.attack(dir);
                        cooldownManager.markUsed(best, step);
                        return;
                    }
                }
                case "SPECIAL" -> {
        ...
        // Nếu không có vũ khí nào sẵn sàng hoặc không thể tấn công, di chuyển rút ngắn khoảng cách
        String path = PathUtils.getShortestPath(gameMap, nodesToAvoid, player, enemy, true);
        if (path != null && !path.isEmpty()) {
            hero.move(path);
        } else {
            System.out.println("Không thể tấn công hoặc di chuyển tới kẻ địch!");
        }
    }
    // --- Gun logic ---
    ...
    // ================== COOLDOWN MANAGER & TIỆN ÍCH ==================
    // Copy từ newCooldown.java
    private static class CooldownManager {
        private static final double STEP_SEC          = 0.5;
        private static final double BONUS_THROWABLE   = 50;
        private static final double BONUS_CLOSE_MELEE = 1.3;
        private static final double BONUS_LOW_USE     = 0.2;

        private static final class Slot {
            Weapon w; int nextStep; int usesLeft;
            Slot(Weapon w,int now){
                this.w=w; this.nextStep=now;
                this.usesLeft=w.getUseCount()<=0?-1:w.getUseCount();
            }
            boolean ready(int step){ return step>=nextStep && usesLeft!=0; }
            void mark(int step){ nextStep=step+cdToStep(w); if(usesLeft>0)usesLeft--; }
            static int cdToStep(Weapon w){ return (int)Math.ceil(w.getCooldown()/STEP_SEC); }
        }

        private final Map<String,Slot> slots = new HashMap<>();
        private int lastInvHash = 0;

        void syncInventory(jsclub.codefest.sdk.model.Inventory inv,int step){
            int h = Objects.hash(inv.getGun(),inv.getMelee(),
                    inv.getThrowable(),inv.getSpecial());
            if(h==lastInvHash) return;
            lastInvHash=h; slots.clear();
            add(inv.getGun(),step); add(inv.getMelee(),step);
            add(inv.getThrowable(),step); add(inv.getSpecial(),step);
        }

        boolean ready(Weapon w, GameMap m){
            Slot s=slots.get(w.getId());
            return s!=null && s.ready(m.getStepNumber());
        }

        Weapon chooseBest(int stepNow, boolean enemyClose){
            double best=-1; Slot pick=null;
            for(Slot s:slots.values()){
                if(!s.ready(stepNow)) continue;
                if(enemyClose && "THROWABLE".equals(s.w.getType().name())) continue;

                double score=dpsScore(s);
                if(enemyClose && "MELEE".equals(s.w.getType().name())) score*=BONUS_CLOSE_MELEE;
                if(!enemyClose && "THROWABLE".equals(s.w.getType().name())) score+=BONUS_THROWABLE;
                if(s.usesLeft>0 && s.usesLeft<=2) score*=1+BONUS_LOW_USE;
        ...
        void markUsed(Weapon w,int step){ Slot s=slots.get(w.getId()); if(s!=null)s.mark(step); }

        private void add(Weapon w,int step){ if(w==null||w.getUseCount()==0) return; slots.put(w.getId(),new Slot(w,step)); }
        private double dpsScore(Slot s){ return (double) s.w.getDamage() /Math.max(1,Slot.cdToStep(s.w)); }
    }
