...
public class Main {
    ...
    private static final String GAME_ID = "111156"; // Nhập Game ID vào đây
...
class FullBotListener implements Emitter.Listener {
    ...
    private Obstacle findNearestChestForComparison(GameMap gameMap, Player player) {
        ...
        if (nearestChest != null) {
            System.out.println("[CHEST COMPARISON] Tìm thấy rương gần nhất: " + nearestChest.getId() +
                    " tại (" + nearestChest.getX() + ", " + nearestChest.getY() +
                    ") - Khoảng cách: " + minDistance);
        }
    ...
    private void handleEnemyHuntingAll(GameMap gameMap, Player player, List<Node> nodesToAvoid) throws IOException {
        ...
        if (hero.getInventory().getThrowable() != null) {
            if (handleThrowableAttack(gameMap, player)) {
                ...
                // Không ném được, di chuyển đến kẻ địch để ném
                handleThrowableMovement(gameMap, player, nodesToAvoid);
                return;
            }
        }
    ...
    // Hàm di chuyển đến kẻ địch để ném throwable
    private void handleThrowableMovement(GameMap gameMap, Player player, List<Node> nodesToAvoid) throws IOException {
        List<Player> enemies = gameMap.getOtherPlayerInfo().stream()
                .filter(e -> e.getHealth() > 0)
                .toList();

        if (enemies.isEmpty()) {
            System.out.println("Không còn kẻ địch nào để ném!");
            return;
        }

        // Nếu đang theo dõi một kẻ địch cụ thể và kẻ đó vẫn còn sống
        if (currentTargetEnemy != null && isEnemyStillAlive(gameMap, currentTargetEnemy)) {
            handleCurrentEnemyTrackingThrowable(player, nodesToAvoid, gameMap);
            return;
        }

        // Tìm kẻ địch gần nhất
        Player nearestEnemy = findNearestEnemy(enemies, player);
        ...
        System.out.println("🎯 Tìm thấy kẻ địch để ném tại vị trí (" + nearestEnemy.getX() + ", " + nearestEnemy.getY() +
                ") - HP: " + nearestEnemy.getHealth());
        ...
        if (pathToEnemy != null) {
            if (pathToEnemy.isEmpty()) {
                // Đã ở cạnh kẻ địch, thử ném lại
                System.out.println("Đã đến gần kẻ địch, thử ném throwable!");
                handleThrowableAttack(gameMap, player);
            } else {
                // Di chuyển đến kẻ địch
                System.out.println("Di chuyển đến kẻ địch để ném: " + pathToEnemy);
                hero.move(pathToEnemy);
            }
        } else {
    ...
    private void handleCurrentEnemyTrackingThrowable(Player player, List<Node> nodesToAvoid, GameMap gameMap) throws IOException {
        if (currentTargetEnemy == null) return;
        ...
        if (distanceToEnemy <= 1) {
            ...
            String pathToEnemy = PathUtils.getShortestPath(hero.getGameMap(), nodesToAvoid, player, currentTargetEnemy, true);
            if (pathToEnemy != null && !pathToEnemy.isEmpty()) {
                System.out.println("Tiếp tục theo dõi kẻ địch để ném tại vị trí mới (" + currentTargetEnemy.getX() + ", " + currentTargetEnemy.getY() + "): " + pathToEnemy);
                hero.move(pathToEnemy);
            } else {
